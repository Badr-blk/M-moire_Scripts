...
qual_liquid = pd.read_csv('QUAL_liquid_filtered.csv')
tickers = qual_liquid['Ticker'].tolist()
price_data = yf.download(tickers, start='2019-01-01', end='2024-12-31', group_by='ticker', auto_adjust=True)

close_prices = pd.concat([price_data[ticker]['Close'].rename(ticker) for ticker in tickers if ticker in price_data.columns], axis=1)
volumes = pd.concat([price_data[ticker]['Volume'].rename(ticker) for ticker in tickers if ticker in price_data.columns], axis=1)

benchmark = yf.download('QUAL', start='2019-01-01', end='2024-12-31', auto_adjust=True)
benchmark['Daily Return'] = benchmark['Close'].pct_change()

years = list(range(2019, 2025))
risk_free_rates = {2019: 0.023, 2020: 0.009, 2021: 0.001, 2022: 0.018, 2023: 0.045, 2024: 0.0469}
...
for year in years:
    ...
    filtered_weights = qual_liquid[qual_liquid['Ticker'].isin(filtered_tickers)].set_index('Ticker')['New Weight']
...
print("\n✅ Résultats de performance QUAL filtré :")
print(filtered_performance_df)
filtered_performance_df.to_csv('Analyse_QUAL_Filtré_2019_2024.csv')
print("\n✅ Fichier 'Analyse_QUAL_Filtré_2019_2024.csv' sauvegardé !")
